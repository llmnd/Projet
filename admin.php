...
function initialiserTournoi($mysqli) {
    ...
    for ($i = 0; $i < count($boxeurs) - 1; $i += 2) {
        ...
        $stmt = $mysqli->prepare("INSERT INTO tournoi (ronde, boxeur1_id, boxeur2_id, date_combat, type) VALUES (1, ?, ?, ?, 'round')");
        if (!$stmt) {
...
function reinitialiserStats($mysqli) {
    if (!$mysqli->query("DELETE FROM stats")) {
...
$matchs = $mysqli->query("SELECT t.id, t.ronde, b1.nom AS boxeur1, b2.nom AS boxeur2, a.username AS arbitre, t.date_combat, t.boxeur1_id, t.boxeur2_id, t.arbitre_id, t.type FROM tournoi t JOIN boxeurs b1 ON t.boxeur1_id = b1.id JOIN boxeurs b2 ON t.boxeur2_id = b2.id LEFT JOIN arbitres a ON t.arbitre_id = a.id ORDER BY t.ronde");
...
<body>
    ...
    <table>
        <tr><th>Ronde</th><th>Boxeur 1</th><th>Boxeur 2</th><th>Arbitre</th><th>Date du Combat</th><th>Type</th><th>Action</th></tr>
