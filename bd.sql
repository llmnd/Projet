...
--
-- Table for individual statistics
--

DROP TABLE IF EXISTS `boxeur_stats`;
CREATE TABLE IF NOT EXISTS `boxeur_stats` (
  `id` int NOT NULL AUTO_INCREMENT,
  `boxeur_id` int NOT NULL,
  ...
  PRIMARY KEY (`id`),
  FOREIGN KEY (`boxeur_id`) REFERENCES `boxeurs`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

ALTER TABLE `boxeur_stats` 
ADD COLUMN `decision` INT DEFAULT 0;

-- Update existing rows to initialize the new column
UPDATE `boxeur_stats` 
SET `decision` = 0;
...
CREATE TABLE IF NOT EXISTS `match_stats` (
  `id` int NOT NULL AUTO_INCREMENT,
  `match_id` int NOT NULL,
  `boxeur1_score` int DEFAULT 0,
  `boxeur2_score` int DEFAULT 0,
  `method_victoire` enum('KO', 'TKO', 'decision', 'abandon') DEFAULT NULL,
  PRIMARY KEY (`id`),
...
--
-- Initialize individual statistics for all boxers
--

INSERT INTO `boxeur_stats` (`boxeur_id`) 
SELECT `id` FROM `boxeurs`;